...
test('measured Mic input lives in Performance while gain and suggestion stay in Adjust', () => {
  for (const id of [
    'mic-input-meter',
    'mic-input-value',
...
test('Listen defaults audible at 30% and exposes mute rather than an enable action', () => {
  ...
  assert.equal(listen.includes("t('listen.mutedForMic')"), true);
  assert.equal(listen.includes("t('listen.unmute')"), true);
  assert.equal(listen.includes("t('listen.mute')"), true);
  assert.equal(listen.includes("t('listen.firstInteraction')"), true,
    'default-unmuted intent must acknowledge the mobile autoplay boundary through product copy');
});
...
test('Adjust keeps one flat control layer and thin rails instead of a card wall', () => {
  assert.equal(html.includes('href="/adjust.css"'), true);
  assert.equal(css.includes('border-radius: 0;'), true);
  assert.equal(css.includes('.voice-input-meter'), false,
    'measured input presentation belongs to the persistent performance layer');
  assert.equal(css.includes('.adjust-range::-webkit-slider-runnable-track'), true);
});

test('opening Adjust extends the page without hiding performance or overlaying YouTube', () => {
