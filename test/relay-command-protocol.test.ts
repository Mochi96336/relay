...
test('command protocol selects the extracted low-risk commands and preserves payload identity', () => {
  ...
  assert.equal(seen[0]?.payload, rename);
  assert.equal(seen[1]?.payload, acquire);
  assert.equal(seen[2]?.payload, forceAcquire);
  assert.equal(seen[3]?.payload, intent);
});
...
test('command protocol leaves unextracted commands and malformed envelopes to later routing', () => {
  ...
  assert.equal(protocol.dispatch({}, { type: 'start-take' }), false);
