...
test('severe local WT congestion contributes to packet coverage and spends bounded demotion', async () => {
  ...
  for (let window = 0; window < 4; window += 1) {
    ...
    // Relay receives every packet the browser transport actually admitted.
    // Recovery must nevertheless measure the final media-attempt denominator,
    // so the two locally rejected packets in each trio remain real timeline
    // loss rather than disappearing from the coverage calculation.
    socket.emit({
