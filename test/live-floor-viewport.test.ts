...
test('mobile Live floor keeps dynamic viewport units and viewport panning out of persistent authority', async () => {
  ...
  const startIndex = viewportJs.indexOf('function start()');
  const disposeIndex = viewportJs.indexOf('function dispose()');
  assert.ok(startIndex >= 0 && disposeIndex > startIndex);
  const startBlock = viewportJs.slice(startIndex, disposeIndex);
  const firstViewportListener = startBlock.indexOf("viewport?.addEventListener?.('resize'");
  assert.ok(firstViewportListener > 0);
  const startupAuthority = startBlock.slice(0, firstViewportListener);
  assert.match(startupAuthority, /applyOffset\(0\);/,
    'startup must begin on the stable small-viewport floor');
  assert.equal(startupAuthority.includes('reconcile();'), false,
    'startup must not trust an initial large VisualViewport sample');
  assert.match(startupAuthority, /positiveMotionAuthorized\s*=\s*false/,
    'positive floor motion requires post-start growth evidence');
  assert.match(viewportJs, /nextHeight > previousHeight \+ EPSILON_PX && sawSmallViewport/,
    'viewport growth may authorize downward floor motion only after the safe small viewport was observed');
