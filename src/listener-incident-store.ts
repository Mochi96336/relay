...
export function parseListenerIncidentReport(input: unknown): ListenerIncidentReport | null {
  ...
  const snapshotCapacity = Math.max(
    1,
    Math.min(240, Math.trunc(finiteOrNull(rawFlight.snapshotCapacity) ?? 240)),
  );
  const eventCapacity = Math.max(
    1,
    Math.min(512, Math.trunc(finiteOrNull(rawFlight.eventCapacity) ?? 512)),
  );
...
export class ListenerIncidentStore {
  ...
  private async prune() {
    ...
    if (excess <= 0) return;
    await Promise.all(
      entries.slice(0, excess).map((name) => rm(path.join(this.directory, name), { force: true })),
    );
  }
}
